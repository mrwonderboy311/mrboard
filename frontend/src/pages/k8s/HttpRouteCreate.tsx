...
export default function HttpRouteCreate() {
  const navigate = useNavigate()
  const [loading, setLoading] = useState(false)
  const clusterId = localStorage.getItem('clusterId') || ''
  const [form, setForm] = useState({
    nameSpace: '', routeName: '', gatewayName: '',
    hostnames: '', path: '/', backendService: '', backendPort: '80',
  })
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <h1 className="text-2xl font-bold">创建 HTTP Route</h1>
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.hostnames} onChange={e => update('hostnames', e.target.value)} placeholder="example.com" required />
            </div>
            <div className="space-y-2">
