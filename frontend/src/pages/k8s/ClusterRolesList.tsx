...
import { Search, FileCode, Trash2, Plus, ShieldCheck, Loader2 } from 'lucide-react'
...
export default function ClusterRolesList() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api('/mrboard/clusterroles/v1/Del?clusterId=' + clusterId + '&crName=' + deleteTarget)
      toast.success('删除成功')
...
- apiGroups: ["${formApiGroups || ''}"]
  ...
  const columns: Column<CrItem>[] = [
    {
      key: 'crName', header: '名称', className: 'font-medium', render: (d) => (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
            <ShieldCheck size={14} className="text-primary" />
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-sm truncate">{d.crName}</div>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] text-muted-foreground truncate">{d.rules?.length || 0} 规则</span>
              {d.labels && <span className="text-[10px] text-muted-foreground font-mono truncate">{d.labels}</span>}
            </div>
          </div>
        </div>
      ),
      ...
      key: 'actions', header: '', render: (d) => (
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="YAML"
            onClick={(e) => { e.stopPropagation(); navigate('/k8s/clusterroles/yaml?clusterId=' + clusterId + '&crName=' + d.crName) }}>
            <FileCode size={15} />
          </Button>
          ...
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10" title="删除"
            onClick={(e) => { e.stopPropagation(); setDeleteTarget(d.crName) }}>
            <Trash2 size={15} />
          </Button>
        </div>
      ),
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmDialog
        ...
        title="确认操作"
        description={`确定删除 ClusterRole ${deleteTarget}？`}
        variant="destructive"
