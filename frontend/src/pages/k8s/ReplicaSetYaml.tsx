...
export default function ReplicaSetYaml() {
  const [params] = useSearchParams()
  const navigate = useNavigate()
  const clusterId = params.get('clusterId') || localStorage.getItem('clusterId') || ''
  const nameSpace = params.get('nameSpace') || ''
  const rsName = params.get('rsName') || ''
  const baseQuery = `clusterId=${clusterId}&nameSpace=${nameSpace}&rsName=${rsName}`
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/deploy/v1/ReplicasetYaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {rsName}</h1>
      </div>
