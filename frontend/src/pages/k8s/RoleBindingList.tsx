...
export default function RoleBindingList() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<RbItem | null>(null)
  ...
  useEffect(() => {
    let result = items
    if (searchName) result = result.filter(i => i.rbName.toLowerCase().includes(searchName.toLowerCase()))
    if (nsFilter) result = result.filter(i => i.nameSpace === nsFilter)
    setFiltered(result)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    setOperatingName(deleteTarget.rbName)
    try {
      await api('/mrboard/rolebinding/v1/Del?clusterId=' + clusterId + '&nameSpace=' + deleteTarget.nameSpace + '&rbName=' + deleteTarget.rbName)
      toast.success('删除成功')
...
${subjectsArr.map(s => `- kind: ${s.kind}\n  name: ${s.name}${s.namespace ? '\n  namespace: ' + s.namespace : ''}`).join('\n')}`
  ...
  const columns: Column<RbItem>[] = [
    {
      key: 'rbName', header: '名称', className: 'font-medium', render: (d) => (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
            <LinkIcon size={14} className="text-primary" />
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-sm truncate">{d.rbName}</div>
      ...
      key: 'actions', header: '', render: (d) => (
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="YAML"
            onClick={(e) => { e.stopPropagation(); navigate('/k8s/rolebinding/yaml?clusterId=' + clusterId + '&nameSpace=' + d.nameSpace + '&rbName=' + d.rbName) }}>
            <FileCode size={15} />
          </Button>
  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardContent className="py-3"><div className="flex gap-3 items-center flex-wrap">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle>创建 RoleBinding</DialogTitle></DialogHeader>
      ...
      <ConfirmDialog
        ...
        title="确认操作"
        description={`确定删除 RoleBinding ${deleteTarget?.rbName}？`}
        variant="destructive"
