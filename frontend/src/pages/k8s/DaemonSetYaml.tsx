...
export default function DaemonSetYaml() {
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/ds/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {dsName}</h1>
      </div>
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/ds/v1/ModifyByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
