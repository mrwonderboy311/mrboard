...
export default function HttpRouteYaml() {
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/httproute/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {routeName}</h1>
      </div>
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/httproute/v1/UpdateByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
