...
export default function JobDetail() {
  ...
  const baseQuery = `clusterId=${clusterId}&nameSpace=${nameSpace}&jobName=${jobName}`
  ...
  const fetchDetail = useCallback(async () => {
    setLoading(true)
    try { setDetail(await api<any>('/mrboard/job/v1/Detail?' + baseQuery)) }
    catch (err) { toast.error((err as Error).message) } finally { setLoading(false) }
  }, [baseQuery])

  const fetchPods = useCallback(async () => {
    try {
      const data = await api<ApiResponse<Pod[]>>('/mrboard/pod/v1/List?' + baseQuery + '&resType=job&resName=' + jobName)
      setPods(data.data || [])
    } catch { /* may fail */ }
  }, [baseQuery, jobName])
  ...
  useEffect(() => {
    if (!jobName || !nameSpace) { toast.error('缺少必要参数'); return }
    fetchDetail(); fetchPods()
  }, [fetchDetail, fetchPods, jobName, nameSpace])

  const handleTabChange = (value: string) => {
    if (value === 'events' && events.length === 0) fetchSub('events', '/mrboard/event/v1/List?' + baseQuery + '&objName=' + jobName, (d) => setEvents(d.data || []))
    if (value === 'yaml' && !yaml) fetchSub('yaml', '/mrboard/job/v1/Yaml?' + baseQuery, setYaml)
  }

  const handleDelete = async () => {
    try { await api('/mrboard/job/v1/Del?' + baseQuery); toast.success('删除成功'); navigate('/k8s/job') }
    catch (err) { toast.error((err as Error).message) }
  }

  if (loading) return <div className="flex items-center justify-center py-16 text-muted-foreground">加载中...</div>
  if (!detail) return <div className="flex items-center justify-center py-16 text-muted-foreground">未找到Job详情</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        <ConfirmDialog trigger={<Button variant="destructive"><Trash2 size={14} className="mr-1" />删除</Button>} title="确认删除" description={`确定删除 Job "${jobName}"？`} variant="destructive" onConfirm={handleDelete} />
      </div>
      <Card><CardHeader><CardTitle>基本信息</CardTitle></CardHeader><CardContent>
        <table className="w-full text-sm"><tbody>
          {[
            ['名称', detail.jobName || jobName, '命名空间', nameSpace],
            ['镜像', detail.imageUrl || '', '状态', detail.status || ''],
            ['成功Pod', String(detail.succeeded || 0), '失败Pod', String(detail.failed || 0)],
            ['创建时间', detail.createTime || '', '完成时间', detail.completionTime || ''],
          ].map((row, i) => (
      ...
      <Tabs defaultValue="pods" onValueChange={handleTabChange}>
        ...
        <TabsContent value="pods"><Card><CardContent className="p-0"><Table>
          <TableHeader><TableRow><TableHead>名称</TableHead><TableHead>状态</TableHead><TableHead>重启</TableHead><TableHead>Pod IP</TableHead><TableHead>操作</TableHead></TableRow></TableHeader>
          ...
          : pods.map(pod => (<TableRow key={pod.podName}>
            ...
            <TableCell><Badge variant={pod.podPhase === 'Running' || pod.podPhase === 'Succeeded' ? 'default' : 'destructive'}>{pod.podPhase}</Badge></TableCell>
            <TableCell>{pod.restartCount}</TableCell><TableCell className="font-mono text-xs">{pod.podIp}</TableCell>
            <TableCell><Button variant="outline" size="sm" onClick={() => navigate(`/pod/log?clusterId=${clusterId}&nameSpace=${pod.nameSpace}&podName=${pod.podName}`)}><FileText size={14} className="mr-1" />日志</Button></TableCell>
          </TableRow>))}</TableBody>
        ...
        <TabsContent value="yaml"><Card><CardContent className="py-4">
          <YamlViewer yaml={typeof yaml === 'string' ? yaml : JSON.stringify(yaml, null, 2)} onUpdateUrl={'/mrboard/job/v1/ModifyByYaml?' + baseQuery} onUpdated={fetchDetail} />
