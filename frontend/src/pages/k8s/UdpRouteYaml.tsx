...
export default function UdpRouteYaml() {
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/udproute/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      ...
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/udproute/v1/UpdateByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
