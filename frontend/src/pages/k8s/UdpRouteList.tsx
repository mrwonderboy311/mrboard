...
interface RouteItem { udprouteName: string; nameSpace: string; parentRefs: string; rules: number; createTime: string }
...
export default function UdpRouteList() {
  ...
  const addBackend = () => setBackends([...backends, { service: '', port: '53', weight: '100' }])
  ...
  const resetForm = () => {
    ...
    setFormBackendService(''); setFormBackendPort('53')
    setBackends([{ service: '', port: '53', weight: '100' }])
  ...
  const buildYaml = () => {
    ...
    const backendsYaml = (effectiveBackends.length > 0 ? effectiveBackends : [{ service: 'my-service', port: '53', weight: '100' }]).map(b => {
...
${backendsYaml}`
  ...
  const fetchData = async () => {
    setLoading(true)
    try { const res = await api<{ code: number; data: RouteItem[] }>('/mrboard/udproute/v1/List?clusterId=' + clusterId); setItems(res.data || []) }
    catch (err) { toast.error((err as Error).message) } finally { setLoading(false) }
  }
  useEffect(() => { fetchData() }, [clusterId])
  useEffect(() => { setFiltered(searchName ? items.filter(i => i.udprouteName.toLowerCase().includes(searchName.toLowerCase())) : items) }, [items, searchName])
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    setOperatingName(deleteTarget.udprouteName)
    try { await api('/mrboard/udproute/v1/Delete?clusterId=' + clusterId + '&nameSpace=' + deleteTarget.nameSpace + '&udprouteName=' + deleteTarget.udprouteName); toast.success('删除成功'); setDeleteTarget(null); fetchData() }
  ...
  const handleCreate = async () => {
    ...
    try {
      if (createTab === 'form') {
        ...
        await api('/mrboard/udproute/v1/Create', { method: 'POST', body: JSON.stringify({ clusterId, nameSpace: formNamespace, routeName: formName, gatewayName: formGatewayName, backendService: formBackendService, backendPort: Number(formBackendPort) }) })
      } else {
      ...
      key: 'name', header: '名称', className: 'font-medium', render: (d) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="font-semibold text-sm truncate">{d.udprouteName}</div>
      ...
      key: 'actions', header: '', render: (d) => (
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="详情"
            onClick={(e) => { e.stopPropagation(); navigate('/k8s/udproute/detail?clusterId=' + clusterId + '&nameSpace=' + d.nameSpace + '&udprouteName=' + d.udprouteName) }}>
            <Eye size={15} />
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" title="YAML"
            onClick={(e) => { e.stopPropagation(); navigate('/k8s/udproute/yaml?clusterId=' + clusterId + '&nameSpace=' + d.nameSpace + '&udprouteName=' + d.udprouteName) }}>
            <FileCode size={15} />
          </Button>
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="UDP路由" description="UDPRoute 管理">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle>创建 UDPRoute</DialogTitle></DialogHeader>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="text-sm font-medium">名称 *</label><Input value={formName} onChange={e => setFormName(e.target.value)} placeholder="my-udproute" /></div>
                </div>
              ...
              <div className="rounded-lg border p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">UDP 路由支持多后端负载均衡</p>
                  </div>
                ...
                {backends.map((b, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <Input type="number" value={b.port} onChange={e => updateBackend(i, 'port', e.target.value)} placeholder="53" className="w-24" />
                    <div className="w-28">
              ...
              <div className="rounded-lg border p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="text-sm font-medium">后端端口</label><Input type="number" value={formBackendPort} onChange={e => setFormBackendPort(e.target.value)} placeholder="53" /></div>
                </div>
              </div>
            </div>
          ) : (
      ...
      <ConfirmDialog
        ...
        title="确认操作"
        description={`确定删除 ${deleteTarget?.udprouteName}？`}
        variant="destructive"
