...
export default function ServiceYaml() {
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/svc/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {serviceName}</h1>
      </div>
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/svc/v1/ModifyByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
