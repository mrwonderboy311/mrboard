...
import { Search, FileCode, Trash2, Eye, Plus, Lock, Loader2 } from 'lucide-react'
...
export default function SecretList() {
  ...
  const handleDelete = async () => {
    ...
    setOperatingDeploy(deleteTarget.secretName)
      ...
      render: (d) => (
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
            {operatingDeploy === d.secretName ? <Loader2 size={14} className="text-primary animate-spin" /> : <Lock size={14} className="text-primary" />}
          </div>
          <div className="min-w-0">
            <div className="font-semibold text-sm truncate">{d.secretName}</div>
            {operatingDeploy === d.secretName && <span className="text-[11px] text-primary font-medium animate-pulse">{operationProgress}</span>}
            <div className="flex items-center gap-2 mt-0.5">
              <Badge variant="secondary" className="text-[10px] font-mono">{d.nameSpace}</Badge>
              <span className="text-[10px] text-muted-foreground">{d.secretType}</span>
            </div>
          </div>
        </div>
      ),
    ...
    <div className="space-y-4">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle>创建 Secret</DialogTitle></DialogHeader>
      ...
      <ConfirmDialog
        ...
        title="确认删除"
        description={`确定删除 ${deleteTarget?.secretName}？`}
        variant="destructive"
