...
export default function GrpcRouteYaml() {
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/grpcroute/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      ...
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/grpcroute/v1/UpdateByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
