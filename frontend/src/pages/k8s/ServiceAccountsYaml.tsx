...
export default function ServiceAccountsYaml() {
  const [params] = useSearchParams()
  const navigate = useNavigate()
  const clusterId = params.get('clusterId') || localStorage.getItem('clusterId') || ''
  const nameSpace = params.get('nameSpace') || ''
  const saName = params.get('saName') || ''
  const baseQuery = `clusterId=${clusterId}&nameSpace=${nameSpace}&saName=${saName}`
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/serviceaccounts/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {saName}</h1>
      </div>
