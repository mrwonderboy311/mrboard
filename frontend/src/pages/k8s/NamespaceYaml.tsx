...
export default function NamespaceYaml() {
  const [params] = useSearchParams()
  const navigate = useNavigate()
  const clusterId = params.get('clusterId') || localStorage.getItem('clusterId') || ''
  const nameSpace = params.get('nameSpace') || ''
  const baseQuery = `clusterId=${clusterId}&nameSpace=${nameSpace}`
  ...
  useEffect(() => {
    const fetchYaml = async () => {
      try {
        const data = await api<any>('/mrboard/ns/v1/Yaml?' + baseQuery)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">YAML - {nameSpace}</h1>
      </div>
      <YamlViewer yaml={yaml} onUpdateUrl={'/mrboard/ns/v1/ModifyByYaml?' + baseQuery} onUpdated={() => toast.success('更新成功')} />
    </div>
  )
}
