package rbac

import (
	"fmt"
	m "xkube/xadmin/src/models"
)

type GroupController struct {
	CommonController
}

func (this *GroupController) List() {
	page, _ := this.GetInt64("page")
	page_size, _ := this.GetInt64("rows")
	sort := this.GetString("sort")
	order := this.GetString("order")
	if len(order) > 0 {
		if order == "desc" {
			sort = "-" + sort
		}
	} else {
		sort = "Id"
	}
	nodes, count := m.GetGrouplist(page, page_size, sort)
	this.Data["json"] = &map[string]interface{}{"code": 0, "msg": "", "count": count, "data": &nodes}
	this.ServeJSON()
}

func (this *GroupController) Add() {
	g := m.Group{}
	if err := this.ParseForm(&g); err != nil {
		//handle error
		this.Rsp(false, err.Error())
		return
	}
	id, err := m.AddGroup(&g)
	if err == nil && id > 0 {
		this.Rsp(true, "Success")
		return
	} else {
		this.Rsp(false, err.Error())
		return
	}

}

func (this *GroupController) Update() {
	userinfo := this.GetSession("userinfo")
	userip := this.Ctx.Request.Header.Get("x-forwarded-for")
	if userip == "" {
		userip = this.Ctx.Request.RemoteAddr
	}

	g := m.Group{}
	if err := this.ParseForm(&g); err != nil {
		//handle error
		this.Rsp(false, err.Error())
		return
	}
	id, err := m.UpdateGroup(&g)
	if err == nil && id > 0 {
		content := fmt.Sprintf("groupId:%d", g.Id)
		_ = m.InsertLogAudit(userip, userinfo.(m.User).Username, "UpdateGroup", "success", content)

		this.Rsp(true, "Success")
		return
	} else {
		this.Rsp(false, err.Error())
		return
	}

}

func (this *GroupController) Delete() {
	userinfo := this.GetSession("userinfo")
	userip := this.Ctx.Request.Header.Get("x-forwarded-for")
	if userip == "" {
		userip = this.Ctx.Request.RemoteAddr
	}

	Id, _ := this.GetInt64("Id")
	status, err := m.DelGroupById(Id)
	if err == nil && status > 0 {
		content := fmt.Sprintf("groupId:%d", Id)
		_ = m.InsertLogAudit(userip, userinfo.(m.User).Username, "DelGroup", "success", content)
		this.Rsp(true, "Success")
		return
	} else {
		this.Rsp(false, err.Error())
		return
	}
}
