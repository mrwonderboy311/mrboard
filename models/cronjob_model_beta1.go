...
// CronjobListBeta1 retrieves a list of CronJobs from a Kubernetes cluster using the v1beta1 API
// CronjobListBeta1使用v1beta1 API从Kubernetes集群中检索CronJobs列表
//
// Parameters:
...
// []Cronjob - CronJobs列表
// error - 发生的任何错误
func CronjobListBeta1(kubeconfig, namespace, cronjobName string, labelsKey, labelsValue string) ([]Cronjob, error) {
	if namespace == "" {
	...
	//xList, err := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(namespace).List(context.TODO(), listOptions) //yt-pcauto ok,zx-pcauto 异常：the server could not find the requested resource
...
// CronjobDetailBeta1 retrieves detailed information about a specific CronJob using the v1beta1 API
// CronjobDetailBeta1使用v1beta1 API检索特定CronJob的详细信息
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
...
// error - 发生的任何错误
func CronjobDetailBeta1(kubeconfig, namespace, cronjobName string) (*Cronjob, error) {
	//BatchV1 会提示the server could not find the requested resource,改成BatchV1beta1 后正常
	//yt-pcauto:BatchV1,zx-pcauto:BatchV1beta1
	// BatchV1 may report "the server could not find the requested resource", switching to BatchV1beta1 works
	// yt-pcauto:BatchV1,zx-pcauto:BatchV1beta1
	cjobClient := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(namespace)
...
// CronjobCreateBeta1 creates a new CronJob in a Kubernetes cluster using the v1beta1 API
// CronjobCreateBeta1使用v1beta1 API在Kubernetes集群中创建新的CronJob
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
...
// 返回:
// error - 发生的任何错误
func CronjobCreateBeta1(kubeconfig string, bodys []byte) error {
	...
	if taskSetCheck == "on" {
		...
		var concPolicy batchv1beta1.ConcurrencyPolicy
		switch concurrencyPolicy {
		case "Replace":
			concPolicy = batchv1beta1.ReplaceConcurrent
		case "Forbid":
			concPolicy = batchv1beta1.ForbidConcurrent
		default:
			concPolicy = batchv1beta1.AllowConcurrent
		}
	...
	jobClient := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(nameSpace)
...
// CronjobModifyBeta1 modifies an existing CronJob in a Kubernetes cluster using the v1beta1 API
// CronjobModifyBeta1使用v1beta1 API修改Kubernetes集群中现有的CronJob
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
// namespace - the namespace where the CronJob is located
...
// 返回:
// error - 发生的任何错误
func CronjobModifyBeta1(kubeconfig, namespace, cronjobName, key, value string) error {
	jobClient := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(namespace)
...
// CronjobYamlModifyBeta1 updates a CronJob using YAML data and the v1beta1 API
// CronjobYamlModifyBeta1使用YAML数据和v1beta1 API更新CronJob
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
...
// 返回:
// error - 发生的任何错误
func CronjobYamlModifyBeta1(kubeconfig string, yamlData []byte) error {
...
// CronjobCloneBeta1 clones a CronJob to another namespace or cluster using the v1beta1 API
// CronjobCloneBeta1使用v1beta1 API将CronJob克隆到另一个命名空间或集群
//
// Parameters:
...
// 返回:
// error - 发生的任何错误
func CronjobCloneBeta1(kubeconfig, namespace, objname, target_clusterid, target_namespace, target_objname string) error {
	cronjob, err := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(namespace).Get(context.TODO(), objname, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	//new cluster
	// 新集群
	NewClient := common.ClientSet(target_clusterid).BatchV1beta1().CronJobs(target_namespace)
...
// CronjobDelBeta1 deletes a CronJob from a Kubernetes cluster using the v1beta1 API
// CronjobDelBeta1使用v1beta1 API从Kubernetes集群中删除CronJob
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
// namespace - the namespace where the CronJob is located
...
// 返回:
// error - 发生的任何错误
func CronjobDelBeta1(kubeconfig, namespace, cronjobName string) error {
	jobClient := common.ClientSet(kubeconfig).BatchV1beta1().CronJobs(namespace)
...
// GetCronjobYamlBeta1 retrieves the YAML representation of a CronJob using the v1beta1 API
// GetCronjobYamlBeta1使用v1beta1 API检索CronJob的YAML表示
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
// namespace - the namespace where the CronJob is located
...
// error - 发生的任何错误
func GetCronjobYamlBeta1(kubeconfig, namespace, cronjobName string) (string, error) {
...
// CronjobLabelsBeta1 manages labels for a CronJob using the v1beta1 API
// CronjobLabelsBeta1使用v1beta1 API管理CronJob的标签
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
// namespace - the namespace where the CronJob is located
...
// error - 发生的任何错误
func CronjobLabelsBeta1(kubeconfig, namespace, cronjobName string, method string, labelsMap map[string]string) ([]LabelsKv, error) {
...
// CronjobRunBeta1 manually triggers a CronJob to run immediately using the v1beta1 API
// CronjobRunBeta1使用v1beta1 API手动触发CronJob立即运行
//
// Parameters:
// kubeconfig - the kubeconfig to use for connecting to the cluster
// namespace - the namespace where the CronJob is located
...
// 返回:
// error - 发生的任何错误
func CronjobRunBeta1(kubeconfig, namespace, cronjobName string) error {
	clientset := common.ClientSet(kubeconfig)
	cronJob, err := clientset.BatchV1beta1().CronJobs(namespace).Get(context.TODO(), cronjobName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	// 创建Job的名字
